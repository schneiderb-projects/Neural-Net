...
public class NeuralNetDriver<T> {
	 ...
	 */
	NeuralNetDriver(T[][] trainingData,Parse<T> parser, IOManager io, FileManager fm, int numHiddenNodes, 
			double learningRate, int batchSize, double testOnlySubset) {
		initializeDriver(trainingData, parser, io, fm, numHiddenNodes, 
	...
	private void initializeDriver(T[][] trainingData,Parse<T> parser, IOManager io, FileManager fm, 
			int numHiddenNodes, double learningRate, int batchSize, double testOnlySubset) {
		...
		p.names = new String[rawData.length];
		for(int i = 0; i < rawData.length; i++)
			p.names[i] = p.outputTToString(rawData[i][1]);
		...
		setTrainAndTestData(testOnlySubset > 0 && testOnlySubset < 1 ? testOnlySubset : -1);
		nn = buildNet();
	}
	...
	private NeuralNet buildNet() {
		nn = new NeuralNet(p.lengthOfInput,p.numOutputs,nodes,learningRate, this);
	...
	private double[][][] parseTrainingData(T[][] data) {
		...
		for(int i = 0; i < data.length; i++) 
			trainingData = p.concat(trainingData, p.parse(data[i][0],p.outputCodes[i]));
	...
	public void train(int numTimes)
	{
		...
		{
			data = selectSubset(dataTrainInput,dataTrainOutput,batchSize);
			dataInput = data[0];
			dataOutput = data[1];
	...
	public void trainByAccuracy(double targetAccuracy)
	{
		...
		while(curAccuracy < targetAccuracy) {
			while(curAccuracy < targetAccuracy) {
				data = selectSubset(dataTrainInput,dataTrainOutput,50);
				dataInput = data[0];
				dataOutput = data[1];
				nn.train(1, dataInput,dataOutput, true);
	...
	private double[][][] selectSubset(double[][] dataInput,double[][] dataOutput,int size) {
		double[][][] returnArray = new double[2][size][];
		int index;
		for(int i = 0; i < size; i++) {
			index = (int)(Math.random()*(dataInput.length));
			if(dataInput[index] == null || dataOutput[index] == null) {
				i--;
				continue;
			}
			returnArray[0][i] = dataInput[index];
			returnArray[1][i] = dataOutput[index];
		}
		return returnArray;
	}
	...
	{
		io.dispEpochTrainByRound(nn.getTrainingRound(), nn.getInput(), nn.getA1(), nn.getA2(), nn.getExpected(), nn.getW1(), 
				nn.getW2(), nn.getB1(), nn.getB2());
	}

	protected void dispEpochTrainByAccuracy()
	{
		io.dispEpochTrainByAccuracy(nn.getTrainingRound(), nn.getInput(), nn.getA1(), nn.getA2(), nn.getExpected(), nn.getW1(), 
				nn.getW2(), nn.getB1(), nn.getB2());
	}

	protected void dispFinal()
	{
		io.dispFinal(nn.getTrainingRound(), nn.getInput(), nn.getA1(), nn.getA2(), nn.getExpected(), nn.getW1(), 
				nn.getW2(), nn.getB1(), nn.getB2());
	}
